...
function configureSpark(){
    ...
    for c in `printenv | perl -sne 'print "$1 " if m/^SPARK_CONF_(.+?)=.*/'`; 
    do 
...
function configureHadoopEnv(){
    echo "Configuring hadoop env.sh"
     for c in `printenv | perl -sne 'print "$1 " if m/^HADOOP_ENV_(.+?)=.*/'`; 
    do 
        name=`echo ${c} | perl -pe 's/___/-/g; s/__/_/g'`
        var="HADOOP_ENV_${c}"
        value=${!var}
        echo "Setting hadoop environment variable $name=$value"
        echo "export $name=$value" >> $HADOOP_CONF_DIR/hadoop-env.sh
    done 
